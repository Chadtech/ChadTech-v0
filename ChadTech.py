...
#### of (0,0,255)
pygame.display.set_icon(pygame.image.load("thumbnail.PNG").convert())
...
greekSet = Char(l_Frown,L_Frown,(set([ letCor['LEFTALT'], letCor['g'], ]),set([ letCor['RIGHTALT'], letCor['g'], ]) ), 255)
...
imPort = Char(l_Frown,L_Frown,(set ([ letCor['LEFTCONTROL'],letCor['i'] ]),set ([ letCor['RIGHTCONTROL'],letCor['i'] ])),255)
exPort = Char(l_Frown,L_Frown,(set ([ letCor['LEFTCONTROL'],letCor['e'] ]),set ([ letCor['RIGHTCONTROL'],letCor['e'] ])),255)
...
porter = {
	...
	'.':punctuation__period,
	',':punctuation__comma,
	'?':punctuation__question,
	';':punctuation__semicolon,
	':':punctuation__colon,
	'!':punctuation__exclaimation,
	"'":punctuation__singlequote,
	'"':punctuation__doublequote,
	'0':numeral__ze,
	'1':numeral__on,
	'2':numeral__tw,
	'3':numeral__th,
		...
		if event.type == pygame.KEYDOWN:
			...
			if whichScript == 'normal':
				...
				if  greekOn==False and slantyOn==False:
					...
					if event.key in whichChar.keys and (not (letCor['LEFTSHIFT'] in keys)) and  (not (letCor['RIGHTSHIFT'] in keys))  and (not (letCor['LEFTALT'] in keys)) and  (not (letCor['RIGHTALT'] in keys)):
						charray.charen.insert(curChar,addChar(whichChar))
						curChar+=1
					...
					if event.key in whichChar.keys and (not (letCor['LEFTSHIFT'] in keys)) and  (not (letCor['RIGHTSHIFT'] in keys)) and (not (letCor['LEFTCONTROL'] in keys)) and  (not (letCor['RIGHTCONTROL'] in keys)):
						charray.charen.insert(curChar,addChar(whichChar))
						curChar+=1
					...
					if event.key in whichChar.keys and (not (letCor['LEFTSHIFT'] in keys)) and  (not (letCor['RIGHTSHIFT'] in keys)) and (not (letCor['LEFTCONTROL'] in keys)) and  (not (letCor['RIGHTCONTROL'] in keys))  and (not (letCor['LEFTALT'] in keys)) and  (not (letCor['RIGHTALT'] in keys)):
						charray.charen.insert(curChar,addChar(whichChar))
						curChar+=1
				...
				for yit in range(len(whichChar.keys)):
					if event.key in whichChar.keys and whichChar.keys.issubset(keys) and (not (letCor['LEFTSHIFT'] in keys)) and  (not (letCor['RIGHTSHIFT'] in keys)):
						charray.charen.insert(curChar,addChar(whichChar))
						curChar+=1
				...
				if event.key in downarrow.keys and ((letCor['LEFTCONTROL'] in keys) or (letCor['RIGHTCONTROL'] in keys)):
					for yit in [0]*lineLen*6:
						if curChar<(len(charray.charen)):
							curChar+=1
			...
			########################################## Saving DOcuments
							...
							if not ('.txt' in saveName):
								...
								for hsy in pagen:
									...
									for yit in hsy:
										...
										yitCou+=1
									##### Print 'P' on the screen
									screen.blit(L_lP,[windX-60,0])	
								...
								pixelCharCodes=[]
								##### Make an array of all the superScript digits, where the even indexecd elements are the character they belong to, and the odd indexed elements are the character digits
								superRay =[]
								...
								for yit in charray.charen:
									if yit[0] in portin:
										saveStr+=portin[yit[0]]
									else: 
										saveStr+='#'
								savedTxt = open(saveName,'w')
								savedTxt.write(saveStr)
								savedTxt.close()
				...
				for yit in range(len(whichChar.keys)):
					if event.key in whichChar.keys[yit] and whichChar.keys[yit].issubset(keys):
						...
						if '.PNG' in openName:
								...
								if whatsGoinOn=='addingChars':
									...
									if (red,green,blue)!=(0,0,0):
										if even==True:
											...
											even=True
							curChar=len(charray.charen)-1
						elif '.txt' in openName:
							openTxt = open(openName,'r')
							openStr = ''
							for line in openTxt:
								openStr+=line
							openRay = []
							...
							for yit in openStr:
								if yit in porter:
									charray.charen.append(addChar(porter[yit]))
								else:
									charray.charen.append(addChar(misc__numbersign))
							curChar=len(charray.charen)-1
						else:
							charray.charen.append(addChar(uppercase__F))
							charray.charen.append(addChar(uppercase__I))
							charray.charen.append(addChar(uppercase__L))
							charray.charen.append(addChar(uppercase__E))
							charray.charen.append(addChar(space))
							charray.charen.append(addChar(uppercase__T))
							charray.charen.append(addChar(uppercase__Y))
							charray.charen.append(addChar(uppercase__P))
							charray.charen.append(addChar(uppercase__E))
							charray.charen.append(addChar(space))
							charray.charen.append(addChar(uppercase__E))
							charray.charen.append(addChar(uppercase__R))
							charray.charen.append(addChar(uppercase__R))
							charray.charen.append(addChar(uppercase__O))
							charray.charen.append(addChar(uppercase__R))
							curChar+=15
						...
						if '.PNG' in openName:
							...
							for yit in range(ySize*xMarg):
								if whatsGoinOn=='addingChars':
									...
									twNum+=(green/16)
									if onNum!=161 and onNum!=0:
										charray.charen.insert(curChar,addChar(charLet[onNum]))
										curChar+=1
									if twNum!=161 and twNum!=0:
										charray.charen.insert(curChar,addChar(charLet[twNum]))
										curChar+=1
									if onNum==161 or twNum==161:
										whatsGoinOn='addingSuperscripts'
										even=True
									...
									if (red,green,blue)!=(0,0,162):
										if (red,green,blue)!=(0,0,0):
											if even==True:
												...
												partSuper+=green*256
												partSuper+=red*65536
												charray.charen[charSpot+startpoint][1].append(charLet[partSuper])
												even=True
									else:
										whatsGoinOn='addingSubscripts'
										even=True
									...
									if (red,green,blue)!=(0,0,0):
										if even==True:
											...
											partSub+=green*256
											partSub+=red*65536
											charray.charen[charSpot+startpoint][2].append(charLet[partSub])
											even=True
						elif '.txt' in openName:
							openTxt = open(openName,'r')
							openStr = ''
				...
				if whichScript == 'superscript':
					...
					if greekOn==False:
						...
						whichChar=uppercase__A
						for yit in whichChar.keys:
							if event.key in yit and yit.issubset(keys):
								charray.charen[curChar-1][1].append(whichChar)
